...
class TemporallyDependentObjectStateEstimatorV2(nn.Module):
    """
    A less-naive estimator for determining an object's pose based on visual observation from a robot's "eye-in-hand"
    camera and arm's (noisy) measurement of its own state. Takes into account temporal dependencies via LSTM module

    Unlike the original TDOSE model, this framework considers separate LSTMs for each sensor modality, before
    concatenating the outputs in a final FC network
    """
    ...
    def __init__(
        ...
        Args:
            ...
            img_hidden_dim (int): size of hidden state for image LSTM

            proprio_hidden_dim (int): size of hidden state for proprioception LSTM
        ...
        """
        # Always run super init first
        super(TemporallyDependentObjectStateEstimatorV2, self).__init__()
        ...
        # Define LSTM nets
        self.img_rnn = nn.DataParallel(nn.LSTM(input_size=input_dim, hidden_size=img_hidden_dim))
        self.proprio_rnn = nn.DataParallel(nn.LSTM(input_size=7, hidden_size=proprio_hidden_dim))
        fc_input_dim = img_hidden_dim + proprio_hidden_dim
        self.fc = nn.DataParallel(torch.nn.Sequential(
            nn.Linear(fc_input_dim, int(fc_input_dim // 4)),
            nn.Linear(int(fc_input_dim // 4), 7)
        ))
        self.sequence_length = sequence_length
        ...
        self.img_rnn_h = None
        self.img_rnn_c = None
        self.proprio_rnn_h = None
        self.proprio_rnn_c = None
        self.img_hidden_dim = img_hidden_dim
        self.proprio_hidden_dim = proprio_hidden_dim
    ...
    def forward(self, img, depth, self_measurement):
        ...
        # Pass image features through image RNN
        if not self.rollout:
            img_rnn_h, _ = self.img_rnn(features)  # Output shape (S, N, img_hidden_dim)
        else:
            img_rnn_h, (new_img_h, new_img_c) = self.img_rnn(features, (
                self.img_rnn_h, self.img_rnn_c))
            self.img_rnn_h = new_img_h
            self.img_rnn_c = new_img_c

        # Pass proprioceptive measurements through proprioception RNN
        if not self.rollout:
            proprio_rnn_h, _ = self.proprio_rnn(self_measurement)  # Output shape (S, N, proprio_hidden_dim)
        else:
            proprio_rnn_h, (new_proprio_h, new_proprio_c) = self.proprio_rnn(self_measurement, (
                self.proprio_rnn_h, self.proprio_rnn_c))
            self.proprio_rnn_h = new_proprio_h
            self.proprio_rnn_c = new_proprio_c

        # Concat each RNN output to form the output
        rnn_h = torch.cat((img_rnn_h, proprio_rnn_h), dim=-1)      # Output shape (S, N, (img+proprio)_hidden_dim)
    ...
    def reset_initial_state(self, batch_size):
        ...
        Args:
            batch_size (int): Batch size currently being run
        """
        self.img_rnn_h = torch.zeros((1, batch_size, self.img_hidden_dim), requires_grad=True)
        self.img_rnn_c = torch.zeros((1, batch_size, self.img_hidden_dim), requires_grad=True)
        self.proprio_rnn_h = torch.zeros((1, batch_size, self.proprio_hidden_dim), requires_grad=True)
        self.proprio_rnn_c = torch.zeros((1, batch_size, self.proprio_hidden_dim), requires_grad=True)
